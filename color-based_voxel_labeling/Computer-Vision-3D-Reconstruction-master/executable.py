...
from modules.io import save_voxel_shape
...
camera = Camera(glm.vec3(0, 0, 0), pitch=0, yaw=0, speed=40)
saveVoxelFlag = False
...
def main():
    ...
    while not glfw.window_should_close(window):
        ...
        if shouldStartVideo:
            positions, colors, stream_ended = set_voxel_positions(config['world_width'], config['world_height'], config['world_depth'])
...
def key_callback(window, key, scancode, action, mods):
    global saveVoxelFlag, shouldStartVideo, shouldExitLoop
    if key == glfw.KEY_ESCAPE and action == glfw.PRESS:
        glfw.set_window_should_close(window, glfw.TRUE)
    if key == glfw.KEY_G and action == glfw.PRESS and not shouldStartVideo:
        global cube
        positions, colors, stream_ended = set_voxel_positions(config['world_width'], config['world_height'], config['world_depth'])
        ...
        cube.set_multiple_positions(positions, colors)
    if key == glfw.KEY_M and action == glfw.PRESS and not saveVoxelFlag:
        saveVoxelFlag = True
        save_voxel_shape("../voxel", voxel_visibility)
        saveVoxelFlag = False
    if key == glfw.KEY_C and action == glfw.PRESS and not shouldStartVideo:
        shouldStartVideo = True
    elif key == glfw.KEY_C and action == glfw.PRESS and shouldStartVideo:
        shouldStartVideo = False
